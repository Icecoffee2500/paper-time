...
### 노트 편집기 (2026-09-08 결정)
- **라이브 프리뷰**: 커서가 있는 줄만 마크다운 원문을 보이고, 나머지 줄은 뜻을 보인다(제목은 크게, `- `는 불릿, `$…$`는 수식). 고칠 수 있는 것만 보여준다는 규칙. Obsidian과 같은 방식.
- **Raw 토글**: 노트 전체를 원문으로 놓고 고칠 수 있다. 링크나 수식을 손으로 고칠 때 쓴다.
- **복사**: 화면의 글자가 아니라 마크다운을 준다. 수식은 `$x^2$`로, 위키링크는 `[[id|제목]]`으로 나간다.
- **자동화**: Return이 리스트를 잇고(빈 항목이면 리스트를 끝낸다), Tab/⇧Tab이 항목을 들여쓴다. 체크박스·인용·번호 목록 포함.
- **서체**: 라틴·수식은 STIX Two Text(수식용 STIX Two Math와 한 가족), 한글은 Pretendard. 둘 다 없으면 New York / Apple SD Gothic Neo로 내려간다. 캐스케이드로 붙여 한 줄에 두 문자가 섞여도 메트릭이 하나다.
- **`[[` 자동완성**: 시스템 completion 메뉴 대신 자체 패널(제목+식별자, ↑↓·Return·Esc).

### Ultracopy (2026-09-08 결정)
드래그한 구절을 복사하되 수식은 LaTeX으로 되돌린다(⌘⇧C, 선택 바의 함수 버튼).

**PDFKit이 주는 텍스트를 쓰지 않는다.** TeX으로 조판된 논문은 폰트의 ToUnicode가 없거나 틀려서
φ가 ε로, ‖가 ↑로 들어온다. 대신 페이지가 **실제로 그리는 것**을 읽는다:

1. `PDFContentScanner` — 내용 스트림을 직접 훑어(`CGPDFScanner`) 글리프마다 **코드·폰트·크기·좌표**를
   얻고, 분수선·근호처럼 도형으로 그려져 텍스트에 아예 없는 **얇은 채운 사각형/선**도 모은다.
2. `TeXGlyphNames` — 폰트의 `Differences`가 주는 **글리프 이름**(`summationdisplay`, `bardbl`,
   `parenleftBig`)을 LaTeX 명령으로 되돌린다. 이름이 없으면 Computer Modern의 표준 인코딩
   (CMMI/CMSY/CMEX/CMR/MSBM)으로 되돌린다 — 1979년 이후 자리가 바뀐 적이 없다.
3. `MathTranscriber` — **배치를 구조로 읽는다.** 선 위/아래에 뭔가 있으면 `\frac`, 큰 기호 위아래에
   작은 것이 쌓여 있으면 `\sum_{}^{}`, 작아지고 기준선에서 벗어나면 첨자, 위에 얹힌 것은 `\hat`,
   같은 자리에 세로로 반복되는 조각은 하나의 `\|`. 사람이 렌더링을 보고 적는 순서 그대로다.
4. 산문 줄과 수식 줄은 폰트 비율로 가른다. 수식 줄은 2차원이므로 줄 단위가 아니라 **덩어리로** 읽는다.

검증(V-JEPA 2, 식 (2)): 논문의 원문과 `\hat` 하나를 빼고 일치한다.
`$$\mathcal{L}_{\text{teacher-forcing}}(\phi):=\frac{1}{T}\sum_{k=1}^T\|z_{k+1}-z_{k+1}\|_1=…$$`

한계: 글리프 이름도 표준 인코딩도 없는 PDF(스캔본, 일부 워드 출력물)는 PDFKit 텍스트로 물러난다.

### 그래프: 논문을 무엇으로 잇는가 (2026-09-08 결정)
"connecting the dots"가 목적이므로, 연결의 **이유가 서로 다르면 다른 선**으로 그린다. 네 가지뿐이고, 무게도 다르다.

| 선 | 무엇 | 어떻게 찾나 | 무게 |
|---|---|---|---|
| Cites | 한 논문이 다른 논문을 본문에서 언급 | PDF 전문에서 상대 논문의 DOI·arXiv 번호·제목(30자 이상)을 찾음 | 1.0 |
| Notes | 내 노트끼리 `[[링크]]`로 이어져 있고 각 노트의 출처 논문이 다름 | 슬립박스 | 0.85 |
| Author | 같은 사람이 두 논문에 | 성+이름 첫 글자로 동일인 판정 | 0.55 |
| Filed together | 같은 컬렉션이나 같은 태그 | 사용자가 이미 내린 판단 | 0.3 |

- 앞의 둘이 이 기능의 이유다. 나머지 둘은 이미 아는 사실이고, 앞의 둘은 **찾아보지 않으면 영영 안 보이는 연결**이다.
- 전문 스캔은 파일 크기·페이지 수가 그대로면 다시 하지 않는다(`.papertime/citations.json`).
- 라이브러리의 1/4 이상에 등장하는 이름은 아무것도 구분하지 못하므로 저자 선에서 제외한다.
- 배치는 힘 기반(Fruchterman-Reingold). 연결이 하나도 없는 논문은 시뮬레이션에서 빼고 바깥 고리에 둔다 — 숨기지는 않는다.
- 사이드바 Authors 랭킹은 "그 저자가 포함된 논문 수" 기준이며, 같은 동일인 판정을 쓴다.

### 노트: 제텔카스텐 슬립박스 (2026-09-08 결정)
논문별 메모 한 장 대신 **라이브러리 전체가 하나의 슬립박스**다. PKM 도구들이 이미 쓰는 방식을
그대로 따르므로, 같은 폴더를 Obsidian이나 Zettlr로 열어도 그대로 읽힌다.

- 저장 위치: `.papertime/notes/<id>.md`. 한 노트 = 한 파일 = 한 생각.
- 식별자: 작성된 분(`202609081530`). 파일명이자 링크 대상이라 제목을 바꿔도 링크가 끊기지 않는다.
- 머리말: `id / title / paper / tags / created` — 다른 도구가 이미 읽는 `---` 블록.
- 연결: `[[id|제목]]` 위키링크(입력 중 `[[`에서 노트 목록 자동완성), 역링크(Linked from)를 노트 아래에 표시.
- 분류: 본문에 쓴 `#태그`가 곧 태그다(머리말에도 함께 기록). 사이드바 Slip-Box ▸ Notes에서 태그·검색으로 탐색.
- 출처: `paper:`가 있으면 문헌 노트(그 논문의 Note 탭에 뜬다), 없으면 자기 생각 노트.
- 본문 연결: ⌘L로 읽던 구절의 링크(`papertime://anchor?...`)를 커서 위치에 넣는다.
- 편집기: 마크다운과 LaTeX(`$…$`, `$$…$$`)를 쓰는 즉시 조판해 보여준다(수식은 자체 조판기, 외부 의존성 없음).
