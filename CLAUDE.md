...
- **그리기의 나무(프레임·묶음·오토 레이아웃).** 배열은 평평하게 두고(파일이 평평한 목록이고, 렌더러와 PDF 사본이 다 그렇게 걷는다) `parent` 필드로 나무를 읽는다(`SketchTree`). **자식의 좌표도 쪽 좌표다** — 부모 상대가 아니다 — 그래서 옛 렌더러와 PDF 사본은 아무것도 몰라도 된다. 프레임(`.frame`)은 제 상자가 있는 컨테이너(채움·테두리·모서리, `clips`면 넘친 자식을 자르고, `layout`이 있으면 자식을 줄·열로 놓는다), 묶음(`.group`)은 제 상자가 없는 컨테이너(상자는 자식들의 합이고, `points`에 그 합을 캐시해 둔다 — 묶음을 모르는 쪽도 온전한 상자를 보도록). 클릭 규칙은 Figma의 것: 묶음은 통째로 잡히고(가장 바깥 묶음), 프레임은 클릭을 자식에게 흘린다; 더블클릭은 묶음 안으로 들어간다(`entered`). 모든 변경은 `SketchTree.normalized`를 지난다 — 부모 뒤에 자식이 오도록 배열을 재배열하고, 안쪽 프레임부터 레이아웃을 적용하고(`arrange`, hugs면 프레임이 자식에 맞춰 크기를 잡되 왼쪽 위 모서리는 고정), 묶음의 상자를 다시 잡는다. 놓은 자리의 프레임에 자동으로 들어가고(`adopted`: 가운데가 들어가는 가장 위 프레임), 묶음에서는 끌어내지 않는다. 다른 쪽으로 끌어 옮기기는 `finishMove`가 두 쪽의 사이드카를 한 undo 그룹으로 고친다(원소는 id를 유지하고 좌표는 목표 쪽의 것). 글 카드는 `textSizing`이 `autoWidth`(가장 긴 줄 너비, T로 클릭)거나 `autoHeight`(너비 고정·높이 따라감, T로 끌거나 손잡이로 너비를 잡으면 이것이 된다)이고, 옛 파일의 nil은 autoHeight로 읽는다. 글 입력은 Figma처럼 배경 없는 `NSTextView`가 글자만 보이고 카드(채움·테두리)는 `SketchInputView.draw`가 그 아래에 그린다. 새 스타일 필드(`strokeHidden`·`cornerRadius`·`fontSize`·`textAlign`)와 원소 필드(`parent`·`name`·`clips`·`layout`·`textSizing`)는 **값이 있을 때만 쓴다**(둘 다 `encode(to:)`를 직접 구현) — 이 빌드가 그냥 열었던 파일은 바이트까지 그대로 다시 써지고, Portable은 모르는 키를 `extra`로 들고 다닌다. Portable은 나무를 그리고(자르기 포함) 옮기기·지우기·복제에 자손을 따라가게는 했지만, 묶기·프레임 도구·오토 레이아웃 편집은 아직 맥에만 있다.
- **그리기 층의 나머지 규칙.** 도구 줄의 아이콘은 SF Symbols가 아니라 `FigmaIcon`이 직접 그린다(1.6pt 선, 둥근 이음). "끝" 단추는 없다 — 읽는 중에 그린 것(원소나 스트로크)을 클릭하면 `installMarkClickMonitor`의 `hitsDrawing`이 `mode = .draw`로 바꾸고 `updateCanvasInteraction()`을 **즉시** 불러 그 클릭이 오버레이에 떨어진다. 그린 것의 인스펙터는 떠다니지 않고 창의 인스펙터 **Tool 탭**(`InspectorTab.tool`, `SketchInspector(docked: true)`)이며 펜을 들면 그 탭이 앞으로 온다; 인스펙터 탭 이름 넷(Info·Marks·Note·Tool)은 시스템 언어와 무관하게 영어다. 카드의 `$…$`는 `SketchTypesetter.mathProvider`가 있을 때만 수식으로 조판된다 — 맥은 `MathBridge.install()`이 `MathTypesetter`를 꽂고(iOS·Portable·PDF 사본은 쓴 그대로), 수식이 든 문단은 줄바꿈 없이 한 줄이다. 글꼴은 `SketchStyle.fontName`(패밀리 이름; `SketchTypesetter.font(points:name:)`가 그 패밀리가 실제로 있을 때만 쓴다). 모서리는 `Corner` 척도로만 쓴다: 둥근 것 안의 둥근 것은 `Corner.inner(outer, inset:)`(바깥 반경 − 간격)로 동심원을 만든다 — 같은 반경을 안팎에 주면 안쪽이 더 날카롭게 보인다. **다크 모드의 Glass 색은 흰 종이다**(`ReaderConfiguration.effectiveTint`): 곱하기 합성은 어두운 패널 위에서 글자까지 지운다. `--papertime-dark=1`이 창만 다크로 보여 주고, `--papertime-split=1`이 처음 두 논문을 나란히 놓는다.
- **논문 나란히 보기.** `LibraryModel.openPaperIDs`(이 세션에서 연 순서; `Scope.open`이 "열린 논문" 선반)와 `AppModel.split: SplitArrangement?`(왼쪽 열·오른쪽 열, 열마다 위·아래 — 최대 넷). 놓는 자리는 `DockZone` 여섯 개이고, 쪽 영역에 `DockDropDelegate`가 `PaperTransfer` 드롭을 받아 `DockZoneOverlay`로 자리를 미리 비춘다(가운데 40%는 자리가 아니다). 우클릭 메뉴 "나란히 열기"도 같은 `app.dock`. 각 칸은 제 `ReaderScreen`이고, **포커스된 칸만 창의 `ReaderLink`를 빌려 쓴다**(나머지는 `app.paneLink(for:)`) — 그래서 툴바·인스펙터·노트가 그 논문 것이 된다. 그러려고 `ReaderCoordinator.link`가 `var`이고 `updateNSView`가 매번 다시 넣으며, `ReaderScreen`은 링크 객체가 바뀌면 제 세션을 새 링크에 다시 `adopt`한다. 칸 클릭은 클릭 모니터가 `link.activated?()`로 알린다.
